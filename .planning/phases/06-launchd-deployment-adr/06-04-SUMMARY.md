...
tech_stack:
  added: []
  patterns: ["Developer-ID node pinning (content-independent DR) for TCC grant survival", "on-host launchctl verification spike"]
key_files:
  created:
    - deploy/launchd/RUNBOOK.md
  modified: []
decisions:
  - "Operator ran S0-S3 clean under launchctl; deferred S4 (upgrade survival), S5 (no restart loop), S6 (write round-trip) as risk-accepted verification debt"
  - "ADR 001 vault back-reference deferred — ADR 001 not located as a discrete file in the JessOS vault; ADR-005 already carries the forward Supersedes reference"
  - "Runbook avoids the literal brew-prefix copy source; pins a Developer-ID node and documents the in-place re-copy + launchctl kickstart upgrade path"
metrics:
  duration_minutes: 25
...
**Deployment runbook for Developer-ID node pinning, first-grant seeding, install, and in-place upgrade survival; host spike S0–S3 verified clean under `launchctl`, with S4/S5/S6 and the ADR 001 vault back-reference deferred as recorded verification debt.**
...
A deployment runbook covering, in order: pin a Developer-ID node to `~/.local/libexec/of-mcp-node` and verify its identity-based designated requirement via `codesign -d -r-`; seed the first-run Automation grant interactively (launchd has no consent UI); set tokens + `make install`; survive a node upgrade by re-copying the Developer-ID binary in place + `launchctl kickstart`; and the permission-denial behavior (loud `-1743` to `server.err`, no restart loop under `KeepAlive=Crashed-only`). Includes a Mermaid TL;DR of the install/upgrade flow and a Spike Results table. Cross-links ADR-005.
...
The plan's Tasks 2 and 3 are `gate="blocking"` human checkpoints. The operator executed S0–S3 (clean) and made an explicit risk-accepted decision to defer S4, S5, S6, and the ADR 001 vault back-reference. These are recorded as deferred verification debt, not failures. The plan's `must_haves` for S4 PASS, S5, and S6 PASS therefore remain **unverified on-host**; the supporting mechanism (identity-based DR proven by S0) makes survival expected but not yet demonstrated end-to-end.
...
- `deploy/launchd/RUNBOOK.md` — exists, contains Developer-ID + of-mcp-node, no `brew --prefix` copy source, Mermaid TL;DR, Spike Results table
