...
// matching only agent-okay-tagged tasks. The function is added to
...
describe('agentOkayPredicate — PERM-01 predicate compilation', () => {
  it('returns a normalized filter with tags=[agent-okay] and tagsOperator=AND', () => {
    const result = agentOkayPredicate();

    expect(result.tags).toContain('agent-okay');
  ...
  it('emitted OmniJS script contains agent-okay tag name', () => {
  ...
  it('a task without agent-okay tag does NOT satisfy the filter (negative test)', () => {
    ...
    // The predicate requires agent-okay — a filter for an untagged task will differ.
